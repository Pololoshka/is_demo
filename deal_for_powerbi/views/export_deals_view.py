...
def export_deals(request: HttpRequest) -> HttpResponseForbidden | JsonResponse:
    """
    Функция отвечает за сбор данных из битрикса, обработку в нужный вид и
    отправку в PowerBI.
    """
    if request.GET.get('skey') != settings.SECRET_KEY_POWER_BI:
        return HttpResponseForbidden()
    but = (
        BitrixUser.objects.filter(is_admin=True, user_is_active=True)
        .first()
        .bitrix_user_token
    )
    ...
    companies = but.call_list_method(
        "crm.company.list",
    ...
    result = {"deals": deals, "companies": companies, "contacts": contacts}
